        ...
        <>
            {filteredNav.map((item) => (
                <li key={item.id} className="mt-[0.125rem] w-full">
                    <NavLink
                        className={`transition-colors text-[rgb(52,71,103)] font-semibold leading-[1.5] px-[1rem] py-[.675rem] rounded-[.5rem] whitespace-nowrap flex items-center mx-[.5rem] my-0 ${
                            pathname === item.slug && 'bg-[rgba(94,114,228,0.13)]'
                        }`}
                        to={item.slug}
                    >
                        ...
                        <span className="duration-300 opacity-100 ml-[.4rem] pointer-events-none ease">
                            {item.title}
                        </span>
                    </NavLink>
                </li>
...
export default function SideBar() {
    const { nameSidebar, company, driver } = useContext(DataContext);

    return (
        <div className="overflow-hidden touch-auto xl:translate-x-0 xl:ml-[1.5rem] xl:left-0 duration-200 transition-transform shadow-xl antialiased p-0 bg-white rounded-[1rem] overflow-y-auto justify-between items-center flex-wrap -translate-x-full max-w-[18rem] w-full block my-[1rem] z-[990] inset-y-0 fixed">
