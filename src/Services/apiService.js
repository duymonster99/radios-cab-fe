...
const apiServiceFeedback = import.meta.env.VITE_API_URL_FEEDBACK;
...
// service feedback
export const getFeedbackService = async (apiUrl) => {
    const response = await axios.get(`${apiServiceFeedback}/${apiUrl}`);
    return response.data;
};

export const postFeedbackService = async ({ url, data }) => {
    const response = await axios.post(`${apiServiceFeedback}/${url}`, data, {
        headers: {
            'Content-Type': 'multipart/form-data',
        },
    });
...
export const putFeedbackService = async ({ url, data }) => {
    const response = await axios.put(`${apiServiceFeedback}/${url}`, data);
    return response.data;
};

export const deleteFeedbackService = async ({ url }) => {
    const response = await axios.delete(`${apiServiceFeedback}/${url}`);
    return response.data;
};
